...
> **数据声明**：仓库同时包含 30 家/60 个明确标记的 Demo 企业与岗位，以及 2026-07-16 核验的 14 家企业、30 个正式招聘项目或岗位。访客默认只看到 30 天内核验的正式数据；这不代表全网完整覆盖或之后持续开放，投递前必须再次核对企业官方页面。
...
- 30 家/60 个明确标记的 Demo 企业与岗位，以及 14 家/30 个截至 2026-07-16 核验的正式发布基线
...
| `CRAWLER_DAILY_HOUR` | 可信官方适配器每日执行小时（调度时区） | `5` |
| `CRAWLER_TIMEZONE` | 每日同步使用的 IANA 时区 | `Asia/Shanghai` |
...
4. 点击“立即同步”验证配置；通用解析发现的岗位会出现在待核验队列。只有代码内置、限定官方域名的结构化适配器可自动刷新核验状态。
5. 核验后用户可点击“前往官方投递”，新窗口将打开来源提供的官方 URL。

仓库预置人工核验的发布基线和官方跳转链接；启动后百度官方结构化适配器每天 `05:00`（`Asia/Shanghai`）检查一次，更新前一自然日发布或变更的校招岗位并刷新仍在官方列表中的岗位状态，其他来源不会自动启用。宿主机和 Docker 必须在该时段运行；若错过执行时间，backend 下次启动时会立即补跑。新增自动来源仍需逐个确认页面结构与许可；若 robots.txt 禁止、地址指向内网、需要登录/验证码、响应超限或格式不受支持，本次运行会安全失败并记录原因。当前调度器适用于本地单个 backend 进程。

Compose 的 `3000`、`8000`、`5432` 和 `6379` 端口均只绑定宿主机 `127.0.0.1`。业务应用、数据库、缓存和上传文件全部保存在本机；自动招聘同步会按上述计划主动访问已批准的企业官方页面，因此“纯本地运行”不等于完全断网运行。
...
本项目采用 [Apache License 2.0](LICENSE)。平台提供的是求职管理工具，不能替代企业官方招聘信息、录用通知或专业意见；企业名称、商标和招聘内容仍归各自权利人所有。

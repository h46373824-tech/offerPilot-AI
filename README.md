...
仓库预置人工核验的发布基线和官方跳转链接；启动后百度官方结构化适配器每 24 小时低频检查一次，其他来源不会自动启用。新增自动来源仍需逐个确认页面结构与许可；若 robots.txt 禁止、地址指向内网、需要登录/验证码、响应超限或格式不受支持，本次运行会安全失败并记录原因。当前调度器适用于本地单个 backend 进程。

...
百度官方校招页使用专用结构化适配器每天 `05:00`（`Asia/Shanghai`）低频检查一次，以前一自然日为新增名单窗口，同时刷新官方列表中现有岗位的核验状态。只有 `source_type=trusted_official_adapter` 且结构通过专用解析的岗位可自动刷新核验时间；通用 HTML/RSS/JSON 发现仍进入待核验队列。

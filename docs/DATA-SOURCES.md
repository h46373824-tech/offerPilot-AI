...
本文件规定 Demo 数据的使用方式，以及第三阶段本地导入数据必须满足的授权、核验、展示、更新和下线要求。

第三阶段只支持管理员主动上传 UTF-8 CSV，不提供爬虫或第三方自动采集。导入批次会记录来源、文件名、行数和错误摘要；企业按规范化名称、岗位按“企业 + 岗位名称”去重，新增或更新记录都必须重新人工核验。

## 2. 第一阶段数据声明

仓库内置的数据全部用于开发、测试和 UI 演示：
